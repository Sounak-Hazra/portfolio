...
const FullScreenphoto = ({setIsFullScreen,isFullPhoto, imgSrc}) => {
    console.log(imgSrc)
    return (
        <button onClick={()=>setIsFullScreen(false)} className={` ${isFullPhoto ? "opacity-1 pointer-events-auto " : "opacity-0 pointer-events-none"} w-full h-full absolute z-20 flex items-center justify-center backdrop-blur-sm top-0 left-0 transition-all duration-300 ease-in-out`}>
